...
import { formatBalance, formatGasPrice, formatGasPrice1 } from "../../zhien/transform";
...
const Buy2: React.FC<Buy2Props> = ({
  ...
  const handleSendTransaction = async () => {
    const amountToSend = parseFloat(inputValue);
    await sendTransaction(amountToSend);
  };
    ...
    >
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        >
          ...
          >
            <Typography variant="h1" sx={{ fontSize: "13px", fontWeight: "normal" }}>
              正在购买 ZHEN
            </Typography>
          </Box>
          ...
          >
            ¥{" "}
            {(
              Number(formatBalance(inputValue?.toString() || "0")) *
              (goldToCNYPrice || 0)
            ).toFixed(3)}{" "}
            CNY
          </Typography>
        ...
        >
          ...
          >
            <CompleteButtons onReturn={handleReturn} onComplete={handleComplete} sendTransaction={handleSendTransaction} />
          </Box>
        </Box>
      </Box>
    </Menu>
